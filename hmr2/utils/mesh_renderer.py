...
class MeshRenderer:
    ...
    # right_bbox_keypoints, left_bbox_keypoints,
    def visualize_tensorboard(self, vertices, camera_translation, images, images_right, images_left, pred_keypoints, gt_keypoints, right_bbox_keypoints, left_bbox_keypoints, focal_length=None, nrow=9, padding=2):
        images_np = np.transpose(images, (0,2,3,1))
        images_right_np = np.transpose(images_right, (0,2,3,1))
        images_left_np = np.transpose(images_left, (0,2,3,1))
        rend_imgs = []
        ...
        right_bbox_keypoints[:, :, :-1] = self.img_res * (right_bbox_keypoints[:, :, :-1] + 0.5)
        left_bbox_keypoints[:, :, :-1] = self.img_res * (left_bbox_keypoints[:, :, :-1] + 0.5)
        ...
        for i in range(vertices.shape[0]):
            ...
            gt_right_keypoints_img = render_hand_keypoints(255 * images_right_np[i], right_bbox_keypoints[i]) / 255
            gt_left_keypoints_img = render_hand_keypoints(255 * images_left_np[i], left_bbox_keypoints[i]) / 255
            ...
            rend_imgs.append(torch.from_numpy(images_right[i]))
            rend_imgs.append(torch.from_numpy(images_left[i]))       
            rend_imgs.append(torch.from_numpy(gt_right_keypoints_img).permute(2,0,1))
            rend_imgs.append(torch.from_numpy(gt_left_keypoints_img).permute(2,0,1))            
