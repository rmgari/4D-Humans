...
class HMR2(pl.LightningModule):
    ...
    def tensorboard_logging(self, batch: Dict, output: Dict, step_count: int, train: bool = True, write_to_summary_writer: bool = True) -> None:
        ...
        images_right = batch['img_right']
        images_right = images_right * torch.tensor([0.229, 0.224, 0.225], device=images_right.device).reshape(1,3,1,1)
        images_right = images_right + torch.tensor([0.485, 0.456, 0.406], device=images_right.device).reshape(1,3,1,1)

        images_left = batch['img_left']
        images_left = images_left * torch.tensor([0.229, 0.224, 0.225], device=images_left.device).reshape(1,3,1,1)
        images_left = images_left + torch.tensor([0.485, 0.456, 0.406], device=images_left.device).reshape(1,3,1,1)                
        ...
        gt_right_bbox_keypoints_2d = batch['keypoints_2d_right_hand_bounding_box'].detach()
        gt_left_bbox_keypoints_2d = batch['keypoints_2d_left_hand_bounding_box'].detach()
        ...
        predictions = self.mesh_renderer.visualize_tensorboard(pred_vertices[:num_images].cpu().numpy(),
                                                               ...
                                                               images_right[:num_images].cpu().numpy(),
                                                               images_left[:num_images].cpu().numpy(),
                                                               pred_keypoints_2d[:num_images].cpu().numpy(),
                                                               gt_keypoints_2d[:num_images].cpu().numpy(),
                                                               gt_right_bbox_keypoints_2d[:num_images].cpu().numpy(),
                                                               gt_left_bbox_keypoints_2d[:num_images].cpu().numpy(),
