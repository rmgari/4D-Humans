...
def get_example(img_path: str|np.ndarray, center_x: float, center_y: float,
                center_right_x: float, center_right_y: float,
                center_left_x: float, center_left_y: float,
                width: float, height: float,
                width_right: float, height_right: float,
                width_left: float, height_left: float,
    ...
    right_hand_patch_cv, trans_right = generate_image_patch_cv2(cvimg,
    ...
    left_hand_patch_cv, trans_left = generate_image_patch_cv2(cvimg,
    ...
    image_right = right_hand_patch_cv.copy()
    image_left = left_hand_patch_cv.copy()
    if is_bgr:
        image = image[:, :, ::-1]
    ...
    img_patch_right = convert_cvimg_to_tensor(image_right)
    img_patch_left = convert_cvimg_to_tensor(image_left)     

     
    if center_right_x == 0 and center_right_y == 0 and height_right == 0 and width_right == 0:
        img_patch_right = np.zeros(img_patch_right.shape)
    
    if center_left_x == 0 and center_left_y == 0 and height_left == 0 and width_left == 0:
        img_patch_left = np.zeros(img_patch_left.shape)
    ...
    for n_c in range(min(img_channels, 3)):
        ...
        img_patch_right[n_c, :, :] = np.clip(img_patch_right[n_c, :, :] * color_scale[n_c], 0, 255)
        img_patch_left[n_c, :, :] = np.clip(img_patch_left[n_c, :, :] * color_scale[n_c], 0, 255)
        if mean is not None and std is not None:
            img_patch[n_c, :, :] = (img_patch[n_c, :, :] - mean[n_c]) / std[n_c]
            img_patch_right[n_c, :, :] = (img_patch_right[n_c, :, :] - mean[n_c]) / std[n_c]
            img_patch_left[n_c, :, :] = (img_patch_left[n_c, :, :] - mean[n_c]) / std[n_c]
    if do_flip:
        ...
        temp_right_patch = img_patch_right.copy()
        img_patch_right = img_patch_left
        img_patch_left = temp_right_patch
        temp_trans_right = trans_right.copy()
        trans_right = trans_left
        trans_left = temp_trans_right

    keypoints_2d_right_hand_bounding_box = keypoints_2d_right_hand.copy()
    for n_jt in range(len(keypoints_2d_right_hand_bounding_box)):
        keypoints_2d_right_hand_bounding_box[n_jt, 0:2] = trans_point2d(keypoints_2d_right_hand_bounding_box[n_jt, 0:2], trans_right)
    keypoints_2d_right_hand_bounding_box[:, :-1] = keypoints_2d_right_hand_bounding_box[:, :-1] / patch_width - 0.5

    keypoints_2d_left_hand_bounding_box = keypoints_2d_left_hand.copy()
    for n_jt in range(len(keypoints_2d_left_hand_bounding_box)):
        keypoints_2d_left_hand_bounding_box[n_jt, 0:2] = trans_point2d(keypoints_2d_left_hand_bounding_box[n_jt, 0:2], trans_left)
    keypoints_2d_left_hand_bounding_box[:, :-1] = keypoints_2d_left_hand_bounding_box[:, :-1] / patch_width - 0.5 
    ...
    if not return_trans:
        return img_patch, img_patch_right, img_patch_left, keypoints_2d, keypoints_2d_right_hand, keypoints_2d_left_hand, keypoints_2d_right_hand_bounding_box, keypoints_2d_left_hand_bounding_box, keypoints_3d, smplh_params, has_smplh_params, img_size
    else:
        return img_patch, img_patch_right, img_patch_left, keypoints_2d, keypoints_2d_right_hand, keypoints_2d_left_hand, keypoints_2d_right_hand_bounding_box, keypoints_2d_left_hand_bounding_box, keypoints_3d, smplh_params, has_smplh_params, img_size, trans, trans_right, trans_left
