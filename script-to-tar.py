...
def main():
    ...
    TAR_DIR = 'do_not_reference/dataset_tars/h36m-train'
    FINAL_DIR = 'h36m-train'
    os.makedirs(FINAL_DIR, exist_ok=True)
    for tar_file_name in os.listdir(TAR_DIR):
        if tar_file_name == 'metadata.tar' or os.path.exists(os.path.join(FINAL_DIR, tar_file_name)):
            continue        
        if tar_file_name.endswith('.tar'):
            tar_file_path = os.path.join(TAR_DIR, tar_file_name)

            # Create a temporary directory to extract the tar content
            with tempfile.TemporaryDirectory() as temp_dir:
                print(f"Extracting {tar_file_path} to {temp_dir}...")
                
                # Untar the file
                with tarfile.open(tar_file_path, 'r') as tar:
                    tar.extractall(path=temp_dir)
                print(Path(temp_dir))
                img_paths = [img for end in args.file_type for img in Path(temp_dir).rglob(end)]
                ...
                with tempfile.TemporaryDirectory() as processed_dir:
                    ...
                    for img_path in img_paths:
                        ...
                        new_pyd_path = processed_dir + '/' + img_fn + '.data.pyd'
                        os.makedirs(os.path.dirname(new_pyd_path), exist_ok=True)
                        with open(new_pyd_path, 'wb') as pyd_file:
                            pickle.dump(pyd_data, pyd_file) 
                        
                        shutil.copy(str(img_path), processed_dir)
                        shutil.copy(str(img_path)[:-4] + '.detection.npz', processed_dir)
                    
                    print("finished with", tar_file_name)
                    # processed_dir completed
                    with tarfile.open(FINAL_DIR + '/' + tar_file_name, 'w') as final_tar:
                        final_tar.add(processed_dir, arcname=".")                    
